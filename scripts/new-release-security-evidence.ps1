...
function Assert-ProtectedFile([string]$Path,[string]$ApprovedRoot) {
  ...
  while($cursor){if($cursor.Attributes-band[IO.FileAttributes]::ReparsePoint){throw "Protected path traverses reparse point: $($cursor.FullName)"};if($cursor.FullName.TrimEnd('\')-ieq$root.TrimEnd('\')){break};$cursor=if($cursor.PSIsContainer){$cursor.Parent}else{$cursor.Directory}}
