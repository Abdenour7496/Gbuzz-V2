...
try{
  ...
  $evidenceArgs=@{ResolvedComposeJson="$scratch/compose.json";BuildImageManifest="$scratch/images.txt";SbomPath="$scratch/sbom.json";VulnerabilitySarif="$scratch/scan.json";SignatureReport="$scratch/sigs.json";ProvenanceReport="$scratch/prov.json";PolicyId='attacker-policy';ProtectedConfiguration="$scratch/config.json";OutputPath="$scratch/out.json"}
  Throws {& (Join-Path $root 'scripts\new-release-security-evidence.ps1') @evidenceArgs} 'Protected file ACL inherits'
